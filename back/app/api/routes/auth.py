...
def _create_refresh_token(db: _Session, users_id: int) -> str:
    token = secrets.token_hex(32)
    expires_at = datetime.now(timezone.utc) + timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS)
    db_token = RefreshToken(users_id=users_id, token=token, expires_at=expires_at)
    db.add(db_token)
    db.commit()
    return token
...
def refresh(req: RefreshRequest, db: DB):
    ...
    user = db.query(User).filter(User.id == db_token.users_id).first()
    if not user:
        db.delete(db_token)
        db.commit()
...
@router.post("/logout")
def logout(req: RefreshRequest, db: DB):
    db_token = db.query(RefreshToken).filter(RefreshToken.token == req.refresh_token).first()
    if db_token:
        db.delete(db_token)
        db.commit()
